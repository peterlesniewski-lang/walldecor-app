...
describe('public installation form routes', () => {
  beforeEach(() => {
    mocks.projection.mockReset().mockResolvedValue({ brand: 'WallDecor', number: 'MON-1', submission: { id: 'draft-1' } })
    mocks.autosave.mockReset().mockResolvedValue({ id: 'draft-1', status: 'DRAFT', draftVersion: 1 })
    mocks.submit.mockReset().mockResolvedValue({ id: 'draft-1', status: 'SUBMITTED', revisionNumber: 1 })
    mocks.correction.mockReset().mockResolvedValue({ id: 'draft-2', status: 'DRAFT', revisionNumber: 2 })
  })
  ...
  it.each([null, [], 'wrong', { submissionId: 'draft-1' }, { submissionId: 'draft-1', draftVersion: 0, clientMutationId: 'client-mutation-0001', answers: [], extra: true }])('returns Polish 400 for a non-strict autosave body: %j', async (body) => {
  ...
  it('forwards a valid autosave, submit and correction without authentication', async () => {
    const autosaveResponse = await PATCH(new NextRequest('http://test/api/public/installations/token/autosave', {
      method: 'PATCH', body: JSON.stringify({ submissionId: 'draft-1', draftVersion: 0, clientMutationId: 'client-mutation-0001', answers: [{ questionKey: 'glify', value: 'UNKNOWN' }] }), headers: { 'Content-Type': 'application/json' },
    }), params)
    const submitResponse = await submit(new NextRequest('http://test/api/public/installations/token/submit', {
      method: 'POST', body: JSON.stringify({ submissionId: 'draft-1', draftVersion: 1, clientMutationId: 'submit-mutation-0001' }), headers: { 'Content-Type': 'application/json' },
    }), params)
    ...
    expect(mocks.autosave).toHaveBeenCalledWith(expect.anything(), 'a'.repeat(43), expect.objectContaining({ submissionId: 'draft-1' }))
  })
})
