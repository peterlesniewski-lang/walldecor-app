...
try {
  ...
  await startServer()
  for (const endpoint of ['break-even', 'break-even/settings', 'break-even/sources']) check((await fetch(`${baseUrl}/api/finance/${endpoint}?year=${year}&month=${month}`)).status === 401, `anonymous read ${endpoint}`)
  check((await fetch(`${baseUrl}/api/finance/break-even/settings`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'margin.save', margin: 0.4, effectiveFrom: '2026-09' }) })).status === 401, 'anonymous write')
  ...
  async function saveMargin(value, period = '2026-09') {
    ...
    await page.getByRole('button', { name: 'Zapisz marżę', exact: true }).click()
  ...
  async function saveNet(value) {
    ...
    await page.getByRole('button', { name: 'Zapisz sprzedaż netto', exact: true }).click()
  ...
  async function matchRent() {
    ...
    await page.getByLabel('Faktura za 2026-09 — Czynsz Puławska TEST', { exact: true }).selectOption('part-rent')
    await page.getByRole('button', { name: 'Zapisz przypisanie', exact: true }).click()
  ...
  await page.getByLabel('Utwórz na podstawie faktury', { exact: true }).selectOption('part-rent:PUL')
  ...
  await page.getByRole('button', { name: 'Zapisz koszt stały', exact: true }).click()
  ...
  await page.getByRole('button', { name: 'Odłącz fakturę — Czynsz Puławska TEST', exact: true }).click()
  ...
  await page.getByRole('button', { name: 'Potwierdź usunięcie netto', exact: true }).click()
  ...
  if (db) {
    ...
    if (report.integrity.length !== 1 || report.integrity[0].integrity_check !== 'ok' || report.foreignKeys.length) {
      report.status = 'FAILED'
      report.error = 'SQLite integrity / foreign key check failed'
      process.exitCode = 1
    }
    await db.$disconnect()
  }
