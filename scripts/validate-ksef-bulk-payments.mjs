...
try {
  ...
  for (let i = 0; i < 55; i++) {
    await db.ksefInvoice.create({ data: {
      ...
      status: 'APPROVED', paymentStatus: i === 5 ? 'PAID' : 'UNPAID',
  ...
  console.log('PASS: browser login, selection, currency totals, pagination reset, sticky desktop/mobile toolbar, bulk payment, database audit, preserved dates, idempotent retry and server restart persistence.')
} finally {
  await browser?.close()
  await stop()
  await db.$disconnect()
