...
try {
  ...
  if (!scopeProduct || scopeProduct.productNameSnapshot !== 'Misty Grey' || scopeProduct.productCodeSnapshot !== 'MG-01' || scopeProduct.collectionSnapshot !== 'Misty') throw new Error('SCOPE_SNAPSHOT_READBACK_FAILED')
...
try {
  ...
  assertStatus(snapshotResponse, 201, 'HTTP create template snapshot'); const snapshot = await snapshotResponse.json()
  if (snapshot.templateId !== template.id || snapshot.templateVersion !== 1 || !String(snapshot.schemaJson ?? '').includes('Wywiad o glifach')) throw new Error('HTTP_TEMPLATE_SNAPSHOT_READBACK_FAILED')
  const duplicateSnapshot = await request(baseUrl, admin, `/api/installations/${order.id}/form-snapshot`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ templateId: template.id }) })
  assertStatus(duplicateSnapshot, 400, 'HTTP rejects replacement of immutable template snapshot')
  const detailResponse = await request(baseUrl, admin, `/installations/${order.id}`)
  assertStatus(detailResponse, 200, 'HTTP order detail with form snapshot panel')
  const detailHtml = await detailResponse.text()
  if (!detailHtml.includes('Formularz klienta') || !detailHtml.includes('Wywiad o glifach') || !detailHtml.includes('Niezmienna kopia przypięta do tej karty.')) throw new Error('FORM_SNAPSHOT_PANEL_READBACK_FAILED')
  ...
  const measurementResponse = await request(baseUrl, admin, `/api/installations/${order.id}/rooms/${room.id}/measurements`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ scopeId: scope.id, elementName: 'Szerokość glifu', value: '12.50', unit: 'CM', source: 'CLIENT', authorId: 'foreign-employee', authorContext: 'CLIENT:spoofed' }) })
  assertStatus(measurementResponse, 201, 'HTTP create measurement'); const measurement = await measurementResponse.json()
  if (measurement.source !== 'EMPLOYEE' || measurement.authorId !== null || measurement.actorRole !== 'ADMIN' || !String(measurement.authorContext ?? '').startsWith('ADMIN:') || !measurement.actorUserId) throw new Error('MEASUREMENT_PROVENANCE_SPOOF_CREATE_FAILED')
  const correctedMeasurement = await request(baseUrl, admin, `/api/installations/${order.id}/rooms/${room.id}/measurements/${measurement.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ value: '13.25', source: 'CLIENT', authorId: 'another-foreign-employee', authorContext: 'CLIENT:patched' }) })
  assertStatus(correctedMeasurement, 200, 'HTTP update measurement')
  const correctedMeasurementBody = await correctedMeasurement.json()
  if (correctedMeasurementBody.source !== 'EMPLOYEE' || correctedMeasurementBody.authorId !== null || correctedMeasurementBody.actorRole !== 'ADMIN' || !String(correctedMeasurementBody.authorContext ?? '').startsWith('ADMIN:') || !correctedMeasurementBody.actorUserId) throw new Error('MEASUREMENT_PROVENANCE_SPOOF_PATCH_FAILED')
  ...
  if (!(await roomsResponse.json()).some((row) => row.scopes.some((entry) => entry.scopeProducts.some((item) => item.productNameSnapshot === 'Misty Grey' && item.collectionSnapshot === 'Misty')))) throw new Error('HISTORIC_SCOPE_SNAPSHOT_MISSING')
