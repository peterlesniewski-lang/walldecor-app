...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    if (!parsed.success) return NextResponse.json({ error: 'Dane zapisu formularza są niepoprawne.' }, { status: 400 })
    ...
    if (error instanceof InstallationFormValidationError) return NextResponse.json({ error: 'Dane formularza są niepoprawne.', fieldErrors: error.fieldErrors }, { status: 400 })
    ...
    if (error instanceof SyntaxError) return NextResponse.json({ error: 'Dane zapisu formularza są niepoprawne.' }, { status: 400 })
    throw error
  }
}
