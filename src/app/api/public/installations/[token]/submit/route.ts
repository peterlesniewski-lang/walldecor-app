...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    if (!parsed.success) return NextResponse.json({ error: 'Dane wysłania formularza są niepoprawne.' }, { status: 400 })
    ...
    if (error instanceof InstallationFormValidationError) return NextResponse.json({ error: 'Uzupełnij wymagane widoczne odpowiedzi.', fieldErrors: error.fieldErrors }, { status: 400 })
    if (error instanceof InstallationFormConflictError) return NextResponse.json({ error: 'Formularz został zapisany w nowszej wersji. Odśwież dane i spróbuj ponownie.' }, { status: 409 })
    if (error instanceof SyntaxError) return NextResponse.json({ error: 'Dane wysłania formularza są niepoprawne.' }, { status: 400 })
    throw error
  }
}
