...
import { InstallationCatalogValidationError, createNextInstallationFormTemplateDraft } from '@/lib/installations/catalog-service'
...
export async function POST(_req: Request, { params }: Params) {
  ...
  try {
    return NextResponse.json(await createNextInstallationFormTemplateDraft(prisma, id, session.user.id), { status: 201 })
  } catch (error) {
