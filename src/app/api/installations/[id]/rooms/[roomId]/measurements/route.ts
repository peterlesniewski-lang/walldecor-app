...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  try {
    return NextResponse.json(await addInstallationMeasurement(prisma, roomId, measurementBodyWithoutProvenance(await req.json()), measurementActorFromViewer(session.user.id, access.viewer)), { status: 201 })
  } catch (error) {
