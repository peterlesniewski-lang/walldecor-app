...
function rawCalendarConfiguration(env: CalendarEnvironment): {
  ...
  return {
    ...
    calendarId: isCalendarId(nonEmpty(env.GOOGLE_CALENDAR_ID)) ? nonEmpty(env.GOOGLE_CALENDAR_ID) : null,
...
/**
 * A deliberately non-secret configuration projection for API/UI readiness.
 * Do not add IDs, emails, or credential data here: this result reaches browsers.
 */
export function getInstallationCalendarReadiness(env: CalendarEnvironment = process.env): CalendarReadiness {
  ...
  const adapterCanRun = raw.adapter === 'google'
  ...
  return {
    ...
    ready: raw.enabled && adapterCanRun && credentialsConfigured && calendarConfigured && impersonationConfigured,
  }
}
...
export function getGoogleCalendarConfiguration(env: CalendarEnvironment = process.env): GoogleCalendarConfiguration {
  ...
  return {
    ...
    impersonatedUser: raw.impersonatedUser,
