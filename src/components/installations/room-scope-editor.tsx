...
export function RoomScopeEditor({ orderId, initialRooms, catalog, canEdit }: { orderId: string; initialRooms: Room[]; catalog: Catalog; canEdit: boolean }) {
  ...
  function renderMeasurementEdit(room: Room, measurement: Measurement) {
    ...
    return <form className="grid w-full gap-2 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void saveMeasurement(room, measurement) }}>
      <div className="flex flex-wrap gap-2 sm:col-span-2"><Button type="button" size="sm" variant={draft.kind === 'RECTANGLE' ? 'default' : 'outline'} aria-pressed={draft.kind === 'RECTANGLE'} onClick={() => { editMeasurementValue(measurement, 'kind', 'RECTANGLE'); if (!['MM', 'CM', 'M'].includes(draft.unit)) editMeasurementValue(measurement, 'unit', 'CM') }}>Szerokość × wysokość</Button><Button type="button" size="sm" variant={draft.kind === 'SINGLE' ? 'default' : 'outline'} aria-pressed={draft.kind === 'SINGLE'} onClick={() => editMeasurementValue(measurement, 'kind', 'SINGLE')}>Długość / ilość</Button></div>
      ...
      <div className="flex gap-2 sm:col-span-2"><Button type="submit" size="sm"><Save className="h-3.5 w-3.5" />Zapisz pomiar</Button><Button type="button" size="sm" variant="ghost" aria-label={`Anuluj edycję pomiaru ${measurement.elementName}`} onClick={() => setMeasurementEditing((current) => { const next = { ...current }; delete next[measurement.id]; return next })}><X className="h-3.5 w-3.5" /></Button></div>
    </form>
  }
