...
import styles from './room-scope-editor.module.css'
...
export function RoomScopeEditor({ orderId, initialRooms, catalog, canEdit, onRoomsChanged }: { orderId: string; initialRooms: Room[]; catalog: Catalog; canEdit: boolean; onRoomsChanged?: (rooms: Room[]) => void }) {
  const [rooms, setRooms] = useState(initialRooms)
  const [previousRooms, setPreviousRooms] = useState(initialRooms)
  if (previousRooms !== initialRooms) {
    setPreviousRooms(initialRooms)
  ...
  function renderMeasurementForm(room: Room, scopeId: string | null, key: string, scopeName?: string) {
    ...
    return <form onSubmit={(event) => { event.preventDefault(); void addMeasurement(room, scopeId, key) }} className={styles.measurementForm} data-kind={draft.kind}>
      {isScope && <div className={styles.measurementTools}><Button type="button" size="sm" variant={draft.kind === 'RECTANGLE' ? 'default' : 'outline'} aria-pressed={draft.kind === 'RECTANGLE'} onClick={() => updateMeasurementForm(key, { kind: 'RECTANGLE', secondaryValue: draft.secondaryValue, unit: ['MM', 'CM', 'M'].includes(draft.unit) ? draft.unit : 'CM' }, emptyMeasurementDraft(scopeId ?? '', 'RECTANGLE'))}>Szerokość × wysokość</Button><Button type="button" size="sm" variant={draft.kind === 'SINGLE' ? 'default' : 'outline'} aria-pressed={draft.kind === 'SINGLE'} onClick={() => updateMeasurementForm(key, { kind: 'SINGLE', secondaryValue: '', unit: draft.unit }, emptyMeasurementDraft(scopeId ?? '', 'SINGLE'))}>Długość / ilość</Button></div>}
      <div className={styles.measurementName}><Label htmlFor={`${key}-element`}>Nazwa pomiaru</Label><Input id={`${key}-element`} aria-label={`Nazwa pomiaru ${labelSuffix}`} value={draft.elementName} onChange={(event) => updateMeasurementForm(key, { elementName: event.target.value }, draft)} className="min-h-11" style={inputStyle} /></div>
      <div><Label htmlFor={`${key}-value`}>{draft.kind === 'RECTANGLE' ? 'Szerokość' : 'Długość / ilość'}</Label><Input id={`${key}-value`} aria-label={`${draft.kind === 'RECTANGLE' ? 'Szerokość' : 'Wartość'} pomiaru ${labelSuffix}`} inputMode="decimal" value={draft.value} onChange={(event) => updateMeasurementForm(key, { value: event.target.value }, draft)} className="min-h-11" style={inputStyle} /></div>
      {draft.kind === 'RECTANGLE' && <div><Label htmlFor={`${key}-secondary`}>Wysokość</Label><Input id={`${key}-secondary`} aria-label={`Wysokość pomiaru ${labelSuffix}`} inputMode="decimal" value={draft.secondaryValue} onChange={(event) => updateMeasurementForm(key, { secondaryValue: event.target.value }, draft)} className="min-h-11" style={inputStyle} /></div>}
      <div className={styles.measurementUnit}><Label htmlFor={`${key}-unit`}>Jednostka</Label><select id={`${key}-unit`} aria-label={`Jednostka pomiaru ${labelSuffix}`} value={draft.unit} onChange={(event) => updateMeasurementForm(key, { unit: event.target.value }, draft)} className="min-h-11 w-full rounded-lg border px-3 text-sm" style={inputStyle}>{units.map((unit) => <option key={unit}>{unit}</option>)}</select></div>
      {!isScope && <div className={styles.fullRow}><Label htmlFor={`${key}-scope`}>Przypisz pomiar do zakresu</Label><select id={`${key}-scope`} aria-label={`Zakres pomiaru w ${room.name}`} value={draft.scopeId} onChange={(event) => updateMeasurementForm(key, { scopeId: event.target.value }, draft)} className="min-h-11 w-full rounded-lg border px-3 text-sm" style={inputStyle}><option value="">Pomiar ogólny pomieszczenia</option>{room.scopes.map((scope) => <option key={scope.id} value={scope.id}>{scope.name}</option>)}</select></div>}
      <Button type="submit" aria-label={isScope ? `Zapisz pomiar do ${scopeName}` : 'Zapisz pomiar'} disabled={!valid || busy === `add-measurement-${key}`} className={styles.measurementSubmit} style={{ background: '#A96A20', color: '#fff' }}><Plus />Zapisz pomiar</Button>
    </form>
  }
  ...
  function renderMeasurementEdit(room: Room, measurement: Measurement) {
    ...
    const units = draft.kind === 'RECTANGLE' ? ['MM', 'CM', 'M'] : ['MM', 'CM', 'M', 'M2', 'MB', 'SZT']
    return <form className={styles.measurementForm} data-kind={draft.kind} onSubmit={(event) => { event.preventDefault(); void saveMeasurement(room, measurement) }}>
      <div className={styles.measurementTools}><Button type="button" size="sm" variant={draft.kind === 'RECTANGLE' ? 'default' : 'outline'} aria-pressed={draft.kind === 'RECTANGLE'} onClick={() => { editMeasurementValue(measurement, 'kind', 'RECTANGLE'); if (!['MM', 'CM', 'M'].includes(draft.unit)) editMeasurementValue(measurement, 'unit', 'CM') }}>Szerokość × wysokość</Button><Button type="button" size="sm" variant={draft.kind === 'SINGLE' ? 'default' : 'outline'} aria-pressed={draft.kind === 'SINGLE'} onClick={() => editMeasurementValue(measurement, 'kind', 'SINGLE')}>Długość / ilość</Button></div>
      <label className={styles.measurementName}><span>Nazwa pomiaru</span><Input aria-label={`Nazwa pomiaru ${measurement.elementName}`} value={draft.elementName} onChange={(event) => editMeasurementValue(measurement, 'elementName', event.target.value)} className="min-h-11" style={inputStyle} /></label>
      <label><span>{draft.kind === 'RECTANGLE' ? 'Szerokość' : 'Długość / ilość'}</span><Input aria-label={`${draft.kind === 'RECTANGLE' ? 'Szerokość' : 'Wartość'} pomiaru ${measurement.elementName}`} value={draft.value} onChange={(event) => editMeasurementValue(measurement, 'value', event.target.value)} className="min-h-11" style={inputStyle} /></label>
      {draft.kind === 'RECTANGLE' && <label><span>Wysokość</span><Input aria-label={`Wysokość pomiaru ${measurement.elementName}`} value={draft.secondaryValue} onChange={(event) => editMeasurementValue(measurement, 'secondaryValue', event.target.value)} className="min-h-11" style={inputStyle} /></label>}
      <label className={styles.measurementUnit}><span>Jednostka</span><select aria-label={`Jednostka pomiaru ${measurement.elementName}`} value={draft.unit} onChange={(event) => editMeasurementValue(measurement, 'unit', event.target.value)} className="min-h-11 rounded-lg border px-3" style={inputStyle}>{units.map((unit) => <option key={unit}>{unit}</option>)}</select></label>
      <select aria-label={`Zakres pomiaru ${measurement.elementName}`} value={draft.scopeId} onChange={(event) => editMeasurementValue(measurement, 'scopeId', event.target.value)} className={`min-h-11 rounded-lg border px-3 ${styles.fullRow}`} style={inputStyle}><option value="">Pomiary ogólne pomieszczenia</option>{room.scopes.map((scope) => <option key={scope.id} value={scope.id}>{scope.name}</option>)}</select>
      <div className={styles.measurementTools}><Button type="submit" size="sm"><Save className="h-3.5 w-3.5" />Zapisz pomiar</Button><Button type="button" size="sm" variant="ghost" aria-label={`Anuluj edycję pomiaru ${measurement.elementName}`} onClick={() => setMeasurementEditing((current) => { const next = { ...current }; delete next[measurement.id]; return next })}><X className="h-3.5 w-3.5" /></Button></div>
    </form>
  }

  return <section aria-labelledby="room-scope-heading" className={`mt-7 rounded-2xl border p-4 sm:p-6 ${styles.editor}`} style={{ background: 'var(--wd-white)', borderColor: 'rgba(30, 30, 30, 0.12)', boxShadow: 'var(--card-shadow)' }}>
