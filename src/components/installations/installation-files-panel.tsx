...
type StoredFile = { id: string; purpose: string; questionKey: string | null; roomId: string | null; scopeId: string | null; originalFilename: string; status: string; byteSize: number | null }
type Room = { id: string; name: string; scopes: Array<{ id: string; name: string }> }

export function InstallationFilesPanel({ orderId, initialFiles, rooms, canEdit }: { orderId: string; initialFiles: StoredFile[]; rooms: Room[]; canEdit: boolean }) {
  ...
  async function upload(event: React.FormEvent) {
    ...
    try {
      ...
      setFile(null); setMismatchId(''); await refresh(); setMessage('Plik został dodany.')
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Nie udało się dodać pliku.') } finally { setBusy(false) }
  }
  async function remove(fileId: string) {
    setBusy(true); setMessage('')
    try { const response = await fetch(`/api/installations/${orderId}/files/${fileId}`, { method: 'DELETE' }); if (!response.ok) throw new Error('Nie udało się usunąć pliku.'); await refresh() } catch (error) { setMessage(error instanceof Error ? error.message : 'Nie udało się usunąć pliku.') } finally { setBusy(false) }
  }
  return <section aria-labelledby="installation-files-heading" className="mt-7 rounded-2xl border p-5 sm:p-6" style={{ background: 'var(--wd-white)', borderColor: 'rgba(30,30,30,.12)', boxShadow: 'var(--card-shadow)' }}>
    ...
    {files.length === 0 ? <p className="mt-4 text-sm" style={{ color: 'var(--wd-text-muted)' }}>Brak dodanych plików.</p> : <ul className="mt-4 space-y-2">{files.map((stored) => <li key={stored.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border px-3 py-3 text-sm" style={{ borderColor: 'rgba(30,30,30,.1)' }}><span><strong>{stored.originalFilename}</strong><span className="ml-2 text-xs" style={{ color: 'var(--wd-text-muted)' }}>{stored.purpose === 'INTERNAL_PROJECT' ? 'Projekt' : stored.purpose === 'MISMATCH_EVIDENCE' ? 'Dowód niezgodności' : 'Załącznik klienta'} · {stored.status}</span></span><span className="flex gap-2"><a className="inline-flex items-center gap-1 rounded-md border px-2 py-1 font-semibold" href={`/api/installations/${orderId}/files/${stored.id}`}><FileDown className="h-4 w-4" />Pobierz</a>{canEdit && <Button type="button" size="sm" variant="outline" disabled={busy} onClick={() => void remove(stored.id)} aria-label={`Usuń plik ${stored.originalFilename}`}><Trash2 className="h-4 w-4" /></Button>}</span></li>)}</ul>}
  </section>
}
