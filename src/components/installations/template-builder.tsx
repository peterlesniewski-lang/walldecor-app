...
import { TemplatePathDesigner, type TemplateDraftStatus } from './template-path-designer'
...
export function TemplateBuilder({ initialTemplates }: { initialTemplates: RawTemplate[] }) {
  ...
  const [designerStatus, setDesignerStatus] = useState<TemplateDraftStatus>({ publishable: false, dirty: false })
  ...
  const publishable = useMemo(() => designerStatus.publishable && !designerStatus.dirty && isPublishable(savedQuestions), [designerStatus, savedQuestions])
  const controlsLocked = Boolean(busy) || designerStatus.dirty

  function activateDraft(draftId: string | null, force = false) {
    if (!force && (busy || designerStatus.dirty)) return
    activeDraftIdRef.current = draftId
    setActiveDraftId(draftId)
    setDesignerStatus({ publishable: false, dirty: false })
  }

  const setDraftStatus = useCallback((draftId: string, status: TemplateDraftStatus) => {
    if (activeDraftIdRef.current === draftId) setDesignerStatus(status)
  }, [])
  ...
  async function createDraft(event: FormEvent<HTMLFormElement>) {
    ...
    if (busy || designerStatus.dirty) return
    setBusy('create'); setMessage('')
    try { const template = await requestJson('/api/installations/templates', { method: 'POST', body: JSON.stringify({ name: templateName }) }) as RawTemplate; replaceTemplate(template); activateDraft(template.id, true); setTemplateName('') }
  ...
  async function publish() {
    if (!activeDraft || !publishable || busy || designerStatus.dirty) return
    const draftId = activeDraft.id
    setBusy('publish'); setMessage('')
    try { const template = await requestJson(`/api/installations/templates/${draftId}/publish`, { method: 'POST' }) as RawTemplate; replaceTemplate(template); if (activeDraftIdRef.current === draftId) activateDraft(null, true); setMessage(`Opublikowano wersję ${template.version}`) }
  ...
  async function nextDraft(template: RawTemplate) {
    ...
    try { const draft = await requestJson(`/api/installations/templates/${template.id}/next-draft`, { method: 'POST' }) as RawTemplate; replaceTemplate(draft); activateDraft(draft.id, true) }
  ...
  return <section aria-labelledby="template-builder-heading" className="rounded-2xl border p-5 sm:p-6" style={{ background: 'var(--wd-white)', borderColor: 'rgba(30, 30, 30, 0.12)', boxShadow: 'var(--card-shadow)' }}>
    ...
    {drafts.length > 0 && <div className="mt-5 max-w-xl"><Label htmlFor="active-template-draft">Wybierz szkic do edycji</Label><select id="active-template-draft" aria-label="Wybierz szkic do edycji" value={activeDraftId ?? ''} onChange={(event) => activateDraft(event.target.value || null)} disabled={controlsLocked} className="mt-2 min-h-11 w-full rounded-lg border px-3 text-sm focus-visible:outline focus-visible:outline-2" style={inputStyle}>{drafts.map((template) => <option key={template.id} value={template.id}>{template.name} · v{template.version} · szkic</option>)}</select></div>}
    {activeDraft && <div className="mt-5 rounded-xl border p-4" style={{ borderColor: 'rgba(169,106,32,.35)', background: 'var(--wd-off-white)' }}>
      <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="data-label" style={{ color: '#8C5718' }}>Szkic v{activeDraft.version}</p><h3 className="font-extrabold" style={{ color: 'var(--wd-dark)' }}>{activeDraft.name}</h3></div><Button type="button" onClick={() => void publish()} disabled={controlsLocked || !publishable} className="min-h-11" style={{ background: '#A96A20', color: '#fff' }}><Send />{busy === 'publish' ? 'Publikowanie…' : `Opublikuj v${activeDraft.version}`}</Button></div>
      <TemplatePathDesigner key={activeDraft.id} draftId={activeDraft.id} questions={savedQuestions} busy={Boolean(busy)} onPersist={persistQuestions} onDraftStatusChange={setDraftStatus} />
    </div>}
    {templates.filter((template) => template.status === 'PUBLISHED').length > 0 && <div className="mt-5 space-y-2">{templates.filter((template) => template.status === 'PUBLISHED').map((template) => <div key={template.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border px-4 py-3" style={{ borderColor: 'rgba(30, 30, 30, 0.12)', background: 'var(--wd-white)' }}><div><span className="num text-xs" style={{ color: '#8C5718' }}>v{template.version}</span><h3 className="font-bold" style={{ color: 'var(--wd-dark)' }}>{template.name}</h3></div><Button type="button" variant="outline" disabled={controlsLocked} onClick={() => void nextDraft(template)} className="min-h-11"><CopyPlus />{busy === `next-${template.id}` ? 'Tworzenie…' : 'Nowy szkic'}</Button></div>)}</div>}
  </section>
}
