...
export function TemplateBuilder({ initialTemplates }: { initialTemplates: RawTemplate[] }) {
  ...
  const publishable = useMemo(() => isPublishable(savedQuestions), [savedQuestions])
  ...
  async function createDraft(event: FormEvent<HTMLFormElement>) {
    ...
    try { const template = await requestJson('/api/installations/templates', { method: 'POST', body: JSON.stringify({ name: templateName }) }) as RawTemplate; replaceTemplate(template); setActiveDraftId(template.id); setTemplateName('') }
  ...
  async function persistQuestions(questions: FormQuestion[]) {
    ...
    try { const template = await requestJson(`/api/installations/templates/${activeDraft.id}`, { method: 'PATCH', body: JSON.stringify({ questions }) }) as RawTemplate; replaceTemplate(template) }
  ...
  async function publish() {
    ...
    try { const template = await requestJson(`/api/installations/templates/${activeDraft.id}/publish`, { method: 'POST' }) as RawTemplate; replaceTemplate(template); setActiveDraftId(null); setMessage(`Opublikowano wersję ${template.version}`) }
  ...
  async function nextDraft(template: RawTemplate) {
    ...
    try { const draft = await requestJson(`/api/installations/templates/${template.id}/next-draft`, { method: 'POST' }) as RawTemplate; replaceTemplate(draft); setActiveDraftId(draft.id) }
  ...
  return <section aria-labelledby="template-builder-heading" className="rounded-2xl border p-5 sm:p-6" style={{ background: 'var(--wd-white)', borderColor: 'rgba(30, 30, 30, 0.12)', boxShadow: 'var(--card-shadow)' }}>
    ...
    {drafts.length > 0 && <div className="mt-5 max-w-xl"><Label htmlFor="active-template-draft">Wybierz szkic do edycji</Label><select id="active-template-draft" aria-label="Wybierz szkic do edycji" value={activeDraftId ?? ''} onChange={(event) => setActiveDraftId(event.target.value || null)} className="mt-2 min-h-11 w-full rounded-lg border px-3 text-sm focus-visible:outline focus-visible:outline-2" style={inputStyle}>{drafts.map((template) => <option key={template.id} value={template.id}>{template.name} · v{template.version} · szkic</option>)}</select></div>}
    {activeDraft && <div className="mt-5 rounded-xl border p-4" style={{ borderColor: 'rgba(169,106,32,.35)', background: 'var(--wd-off-white)' }}>
      ...
      <TemplatePathDesigner questions={savedQuestions} busy={Boolean(busy)} onPersist={persistQuestions} />
    </div>}
