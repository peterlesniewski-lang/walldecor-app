      ...
      {localVisits.map((visit) => {
        ...
        return <article key={visit.id} className="overflow-hidden rounded-xl border" style={{ borderColor: 'rgba(30, 30, 30, 0.12)' }}>
          ...
          {expanded && <div className="border-t p-4" style={{ borderColor: 'rgba(30, 30, 30, 0.12)', background: 'rgba(251, 249, 245, 0.55)' }}>
            ...
            {visit.status !== 'CANCELLED' && visit.syncState.externalUrl && <a href={visit.syncState.externalUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 text-sm font-bold underline underline-offset-4" style={{ color: '#8C5718' }}><ExternalLink className="h-4 w-4" /> Otwórz w Google Calendar</a>}
