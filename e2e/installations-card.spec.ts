...
test('employee completes the simplified card workflow, without manual reload between mutations', async ({ page, browser }, info) => {
  ...
  await scope.getByRole('button', { name: 'Dodaj pracę w Salon', exact: true }).click()
  ...
  await page.getByRole('button', { name: 'Zmień formularz', exact: true }).click()
  await page.getByLabel('Wersja formularza dla zlecenia').selectOption(templateId)
  await page.getByRole('button', { name: 'Zapisz wybór formularza', exact: true }).click()
  await expect(page.getByRole('button', { name: 'Zmień formularz', exact: true })).toBeVisible()
  ...
  await expect(client.getByRole('heading', { name: 'Wysłane odpowiedzi' })).toBeVisible()
  await expect(client.getByRole('button', { name: 'Wyślij formularz', exact: true })).toHaveCount(0)
  await client.reload()
  await expect(client.getByRole('heading', { name: 'Wysłane odpowiedzi' })).toBeVisible()
  expect(await db.installationFormSubmission.count({ where: { orderId, status: 'DRAFT' } })).toBe(0)
