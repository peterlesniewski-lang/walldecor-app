...
test.describe('Installation order workflow', () => {
  test('admin creates, edits, persists and archives an installation order', async ({ page, browser }) => {
    ...
    await expect(page.getByText(/Puławska 19B/)).toBeVisible()
    await expect.poll(async () => (await db.installationOrder.findUniqueOrThrow({ where: { id: orderId } })).addressBuildingNumber).toBe('19B')
    await page.reload()
    await page.getByRole('button', { name: 'Edytuj dane' }).click()
    ...
    await page.getByText('Archiwizacja', { exact: true }).click()
